...
const Busedit = () => {
  ...
  const handleEdit = async (busID) => {
    try {
      const response = await axios.get(`http://busdejavu-backend.vercel.app/buses/${busID}`);
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://busdejavu-backend.vercel.app/buses/${editData.busID}`, editData);
      setBuses((prevBuses) =>
        prevBuses.map((bus) =>
  ...
  const handleDelete = async (busID) => {
    try {
      await axios.delete(`http://busdejavu-backend.vercel.app/buses/${busID}`);
